...
def atari_model(ram_in, num_actions, scope, reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        #x = tf.concat(1,(ram_in[:,4:5],ram_in[:,8:9],ram_in[:,11:13],ram_in[:,21:22],ram_in[:,50:51], ram_in[:,60:61],ram_in[:,64:65]))
        x = ram_in 
        with tf.variable_scope("action_value"):
            x = layers.fully_connected(inputs=x, num_outputs=256, activation_fn=tf.nn.relu)
            x = layers.fully_connected(inputs=x, num_outputs=128, activation_fn=tf.nn.relu)
...
def atari_learn(env, session, num_timesteps):
    ...
    dqn.learn(env, q_func=atari_model, optimizer_spec=optimizer, session=session, 
              ...
              frame_history_len=1, target_update_freq=10000, grad_norm_clipping=10 )
    env.close()
